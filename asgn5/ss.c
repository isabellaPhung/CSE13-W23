...
void ss_make_pub(mpz_t p, mpz_t q, mpz_t n, uint64_t nbits, uint64_t iters){
    ...
    srandom(seed);
    min = nbits/5; // minimum and maximum bits as mentioned in the spec to create p and q with varying bits
    max = (2*nbits)/5;
    max = max-min;
...
void ss_decrypt_file(FILE *infile, FILE *outfile, const mpz_t d, const mpz_t pq){
    ...
    while(bytes != 0){
        ...
        ss_decrypt(m, c, d, pq); 
        int test = mpz_sizeinbase(m, 2)/8; //if the final line is shorter than the previous line, fprintf prints extra characters, this forces fprintf to only print the exact number of characters
